<!-- SIDEBAR RESPONSIF LENGKAP DENGAN KARTU AKUN & POPUP -->
<style>
  :root {
    --sidebar-width: 215px;      
  ...
  aside#sidebar {
    ...
    width: var(--sidebar-width); background: var(--card, #111b16);
    border-right: 1px solid var(--line); display: flex;
    flex-direction: column; z-index: 100; box-shadow: var(--shadow, 0 10px 30px rgba(0,0,0,.3));
  ...
  .mobile-menu-btn:hover { background: var(--card-hover, #17231d); }
  ...
  .brand-text small { font-size: 10px; color: var(--muted, #82988c); display: block; }
  ...
  /* ==========================================================
     KARTU AKUN DI BAWAH SIDEBAR & POPUP MENU
     ========================================================== */
  ...
  .user-profile-btn:hover { background: var(--card-hover, #17231d); }
  ...
  .user-info p { font-size: 9px; margin: 0; color: var(--muted, #82988c); text-overflow: ellipsis; overflow: hidden; }
  
  /* Sembunyikan teks nama & email di kartu bawah saat sidebar diperkecil */
  aside#sidebar.collapsed .user-info { display: none; }
  
  /* Atur agar saat sidebar diperkecil, tombol kartu akun posisinya di tengah */
  aside#sidebar.collapsed .user-profile-btn {
    justify-content: center !important;
    padding: 6px 0 !important;
  }

  /* POPUP MENU PROFIL & LOGOUT (SAAT SIDEBAR NORMAL / LEBAR) */
  .user-popup-menu { 
    position: absolute; bottom: 65px; left: 8px; right: 8px; 
  ...
  .sidebar-toggle-bar { 
    ...
    cursor: pointer; background: rgba(0,0,0,0.1); font-size: 10px; color: var(--muted, #82988c); 
  }
  .sidebar-toggle-bar:hover { background: var(--card-hover, #17231d); color: #fff; }
...
<script>
  ...
  // LOGIKA KLIK KARTU AKUN UNTUK MEMUNCULKAN POP-UP
  if (userProfileBtn && userPopupMenu) {
    userProfileBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      
      // Ambil posisi tombol akun secara dinamis agar pop-up selalu muncul tepat di sampingnya
      const rect = userProfileBtn.getBoundingClientRect();
      
      if (sidebar.classList.contains('collapsed')) {
        // Posisi saat sidebar diperkecil (melayang di sebelah kanan sidebar)
        ...
        // Posisi saat sidebar lebar (di atas tombol akun)
        userPopupMenu.style.position = 'fixed';
